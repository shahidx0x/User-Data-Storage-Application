...
def read_many(db: Session,search:str,limit:int,page:int):
    skip = (page  - 1) * limit
    try:
        children = db.query(Child).filter(Child.first_name.contains(search)).limit(limit).offset(skip).all()
        return children
    except Exception as e:
        print(e)
...
def update_one(db: Session, children_id: int, children_data:ChildrenUpdateSchema):
    try:
        ...
        children_query.filter(Child.id == children_id).update(updated_data,synchronize_session=False) 
        db.commit()
        db.refresh(children)
        return children
    
    except Exception as e:
        raise Exception(f"unable to update children with id {str(children_id)} : {str(e)}")
...
def delete_one(db:Session,children_id:int):
    try:
        ...
        raise Exception(f"unable to delete children with id {str(children_id)} : {str(e)}")
